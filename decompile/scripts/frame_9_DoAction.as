...
function addNewAchievement(ID, name, desc, need, amnt, type, where, secret, redCoin)
{
   ...
   _loc11_.type = type;
   _loc11_.where = where;
   _loc11_.secret = secret;
   _loc11_.redCoin = redCoin;
   achList.push(_loc11_);
}
function checkAchievements()
{
   ...
   {
      if(_root.achList[_loc2_].where == "save")
      {
         achCurrent = _root.save[_root.achList[_loc2_].need];
      }
      else if(_root.achList[_loc2_].where == "root")
      {
         achCurrent = _root[_root.achList[_loc2_].need];
      }
      achRequired = _root.achList[_loc2_].amnt;
      if(!isNaN(achCurrent) && achCurrent >= achRequired && _root.achList[_loc2_].type == "Max" || achCurrent <= achRequired && _root.achList[_loc2_].type == "Min" || !isNaN(_root.save.achEarnTime[_root.achList[_loc2_].ID]))
      {
         if(isNaN(_root.save.achEarnTime[_root.achList[_loc2_].ID]))
         {
            _root.save.achEarnTime[_root.achList[_loc2_].ID] = _root.systemtimenow;
            if(_root.save.showBanner != false)
            {
               _root.accomplishPop.targetX = 10;
               _root.accomplishPop.count = 0;
               _root.accomplishPop.gotoAndStop(2);
               _root.accomplishPop.lolMessage.text = _root.achList[_loc2_].name;
               _root.accomplishPop.rcText.text = "Progress Bar Reward: +" + _root.achList[_loc2_].redCoin / 100 + "%";
               _root.accomplishPop.countText.text = _root.save.maxAchievement + 1 + " / " + _root.totalachievements;
            }
            _root.dispNews(3,"Achievement earned! [" + _root.achList[_loc2_].name + "] (" + (_root.save.maxAchievement + 1) + " / " + _root.totalachievements + ")");
            _root.newAchName = _root.achList[_loc2_].name;
            _root.newAchRedCoin = _root.achList[_loc2_].redCoin;
         }
         _root.awards += 1;
         if(_root.achList[_loc2_].secret == true)
         {
            _root.sawards += 1;
         }
         _root.achRedCoin += _root.achList[_loc2_].redCoin;
      }
      _loc2_ = _loc2_ + 1;
   }
...
function addNewQuest(expired, eventQuest, dailyQuest, questID, questImage, reqLevel, reqQuest, reqQuestRank, questName, descA, descB, need, where, amnt1, amnt2, amnt3, amnt4, questToken, eventToken)
{
   ...
   _loc21_.expired = expired;
   _loc21_.eventQuest = eventQuest;
   _loc21_.dailyQuest = dailyQuest;
   _loc21_.questID = questID;
   _loc21_.questImage = questImage;
   _loc21_.reqLevel = reqLevel;
   _loc21_.reqQuest = reqQuest;
   _loc21_.reqQuestRank = reqQuestRank;
   _loc21_.questName = questName;
   _loc21_.descA = descA;
   _loc21_.descB = descB;
   _loc21_.need = need;
   _loc21_.where = where;
   ...
   _loc21_.amnt3 = amnt3;
   _loc21_.amnt4 = amnt4;
   _loc21_.questToken = questToken;
   _loc21_.eventToken = eventToken;
   mainQuestList.push(_loc21_);
}
function addRandomQuest(chance, difficulty, maxMult, reqLv, reward, rewardQuan, rewardBlueCoin, rewardToken, feature, type, subtype, need, descA, descB)
{
   ...
   _loc16_.chance = chance;
   _loc16_.difficulty = difficulty;
   _loc16_.maxMult = maxMult;
   _loc16_.reqLv = reqLv;
   _loc16_.feature = feature;
   _loc16_.type = type;
   _loc16_.subtype = subtype;
   _loc16_.need = need;
   _loc16_.reward = reward;
   _loc16_.rewardQuan = rewardQuan;
   _loc16_.rewardBlueCoin = rewardBlueCoin;
   _loc16_.rewardToken = rewardToken;
   _loc16_.descA = descA;
   _loc16_.descB = descB;
   questList.push(_loc16_);
   _root.totalquest += 1;
}
function addAchievementR(ID, secret, achName, descA, descB, need, where, amnt)
{
   ...
   _loc9_.ID = ID;
   _loc9_.secret = secret;
   _loc9_.achName = achName;
   _loc9_.descA = descA;
   _loc9_.descB = descB;
   _loc9_.need = need;
   _loc9_.where = where;
   _loc9_.amnt = amnt;
   achListR.push(_loc9_);
}
_root.totalquest = 0;
...
var achListR = [zeroItem];
var questListR = [zeroItem];
